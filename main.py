...
def main():
    ...
    particles += [Particle(mass_p, np.array([np.random.rand()*(x_centre-(thickness/2))+((x_centre+(thickness/2))), height*np.random.rand()]), 2*np.random.uniform(1,2)-1, charge_p, x_range, y_range, type=ParticleType.PROTON) for n in range(NP_A)]
    
    verlet_integrator(particles, poisson_pes, membrane, n_steps, 0.01, system='NVT', gamma=10)
